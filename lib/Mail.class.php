...
function From( $from, $fromname = "" ) {
	...
	$this->xheaders['From'] = $fromname ? "\"$fromname\" <$from>" : $from;
}
...
function Attach( $filename, $basename = "", $filetype = "", $disposition = "inline" ) {
	// TODO : si filetype="", alors chercher dans un tablo de MT connus / extension du fichier
	if( $filetype == "" )
		$filetype = "application/x-unknown-content-type";
		
	$this->aattach[] = array("path" => $filename, "filename" => $basename);
...
function ValidEmail($address){
	...
	return preg_match("#^[a-zA-Z0-9._-]+@[a-zA-Z0-9._-]{2,}\.[a-z]{2,4}$#",$address) ? true : false;
}
...
function _build_attachement()
{
	...
	for( $i=0; $i < count( $this->aattach); $i++ ) {
		...
		$filename = $this->aattach[$i]['path'];
		$basename = $this->aattach[$i]['filename'] ? $this->aattach[$i]['filename'] : basename($filename);
