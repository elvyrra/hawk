<form class="{{ $form->class }} form {{ $form->ajax ? 'ajax-form' : '' }} {{ $form->upload ? 'upload-form' : ''}}" id="{{ $form->id }}" method="{{ $form->method }}" action="{{ $form->action }}" {{ $form->target ? "target='{$form->target}'" : "" }} novalidate autocomplete="{{ $form->autocomplete ? "on" : "off" }}" {{ $form->enctype ? "enctype='{$form->enctype}" : "" }} >
	<input type='hidden' name='_FORM_ACTION_' value='valid'/>
	{if($form->title)}<h2 class='form-title' >{{ $form->title }}</h2>{/if}
