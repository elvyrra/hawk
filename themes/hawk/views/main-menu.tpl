			...
			<ul class="dropdown-menu" role="menu" aria-labelledby="main-menu-title-{{ $menu->id }}" id="main-menu-items-{{ $menu->id }}">
				{foreach($menu->visibleItems as $item)}
					<li role="presentation" id="main-menu-item-{{ $item->id }}">							
						<a role="menuitem" href="{uri action="{$item->action}"}" {if(!empty($item->target))} target="{{ !empty($item->target) ? $item->target : '' }}" {/if}> 
							{{ $item->label }}
						</a>
					</li>					
				{/foreach}					
				...
				<ul class="dropdown-menu" role="menu" aria-labelledby="main-menu-title-admin">
					{foreach($adminMenu as $item)}						
						<li role="presentation">							
							<a role="menuitem" href="{{ $item['url'] }}" target="{{ !empty($item['target']) ? $item['target'] : '' }}" ><i class="fa fa-{{ $item['icon'] }}"></i>{{ $item['label'] }}</a>
						</li>
					{/foreach}
				</ul>
			</div>
		...
		<div class="dropdown pull-right" id="main-menu-user">
			...
			<ul class="dropdown-menu" role="menu" aria-labelledby="main-menu-title-user">
				{foreach($userMenu as $item)}
					<li role="presentation">						
						<a role="menuitem" href="{{ $item['url'] }}" {if(!empty($item['class']))}class="{{ $item['class'] }}"{/if} target="{{ !empty($item['target']) ? $item['target'] : '' }}" ><i class="fa fa-{{ $item['icon'] }}"></i>{{ $item['label'] }}</a>
					</li>					
				{/foreach}					
			</ul>
		</div>
